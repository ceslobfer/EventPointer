    ...
    if (Statistic == "LogFC" | Statistic == "Dif_LogFC" | Statistic == "DRS") {
        ...
        for (mm in seq_len(ncol(Contrast))) {
            ...
            if (Statistic == "LogFC" | Statistic == "Dif_LogFC") {
                ...
                # frame
                Result <- data.frame(EventsFound[,1], 
                                     EventsFound[, 2], EventsFound[, 3], 
                                     EventsFound[, 4], EventsFound[, 5], 
                                     Values1$Tstats, Values1$Pvalues, stringsAsFactors = FALSE)
                
                colnames(Result) <- c("Affy Gene ID", "Gene name", "Event Number", "Event Type", "Genomic Position", "Splicing Z Value", "Splicing Pvalue")
