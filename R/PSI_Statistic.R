...
PSI_Statistic <- function(PSI, Design, Contrast, 
    ...
    V <- Contrast %*% (solve(base::crossprod(Design)) %*% t(Design))
    incrPSI_original <- (base::tcrossprod(V, PSI) + 1)/2
    ncontrastes <- dim(Contrast)[1]
    l <- dim(PSI)[1]
    combboots <- rep(list(matrix(NA, l, nboot)), ncontrastes)  # Initialize matrix for the increase in PSI
    
    for (boot2 in seq_len(nboot)) {
        PSI1 <- PSI[, sample(ncol(PSI), replace = TRUE)]  # Samples the Yb (mixes data)
        output <- base::tcrossprod(V, PSI1)  # Obtain the increase in PSI
    ...
    if (nrow(Contrast) > 1) {
        LocalFDR <- apply(Pvalues, 2, function(X) {
            ...
            salida <- cobs(x = log(result$pvalues + eps), y = result$lfdr, constraint = "incr", pointwise = matrix(c(-1, 0, 1, 1, min(log(result$pvalues + eps), na.rm = TRUE), 0), byrow = TRUE, ncol = 3), lambda = 1, print.mesg = FALSE)
            iix <- which(is.na(X))
            if (length(iix) > 0) {
                jjx <- which(!is.na(X))
                if (length(jjx) > 0) {
        ...
        salida <- cobs(x = log(LocalFDR$pvalues + eps), y = LocalFDR$lfdr, constraint = "incr", pointwise = matrix(c(-1, 0, 1, 1, min(log(LocalFDR$pvalues + eps), na.rm = TRUE), 0), byrow = TRUE, ncol = 3), lambda = 1, print.mesg = FALSE)
