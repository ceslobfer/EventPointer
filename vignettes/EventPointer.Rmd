...
-   *EventPointer_BAM (EP_BAM)* The initial files for this workflow are
...
-   *EventPointer_ST (EP_ST)* In this case, the detection and
...
increment between conditions. ![Figure 3. Output of
CDFfromGTF](Fig1_vignnete_EP.png)

Ambas aproximaciones poseen un elemento común dentro de sus respectivos
flujos de trabajo, y es la creación de un splicing graph.
EventPointerBAM realiza la creación del splicing graph a partir de las
lecturas alineadas usando alineadosres sensibles a las llamadas
junctions reads (Ej. STAR), mientras que EventPointerST utiliza las
anotaciones existentes de exones de las isofórmas de cada gen para crear
el splicing graph, por ello solo requiere las abundancias relativas a
estas isoformas para su flujo de trabajo. ![Figure 4. Splicing
graph](SplicingGraph.png)

For more details on the differences and procedures involved in each
approach, please refer to our paper: \* Romero, Juan P., et al.
"Comparison of RNA-seq and microarray platforms for splice event
detection using a cross-platform algorithm." BMC genomics 19.1 (2018):
703.
[doi:10.1186/s12864-018-5082-2](https://bmcgenomics.biomedcentral.com/articles/10.1186/s12864-018-5082-2)

# EventPointer_BAM

The aim of EventPointer_BAM is to perform detection, classification,
...
En la siguiente tabla podemos ver un ejemplo de los resultados asociados
al análisis diferencial de $\Psi$ de las comparativas indicadas
realizados por la funcion `EventPointerStats_BAM()`. En ella podemos ver
el $\Delta \Psi$ asociado a cada evento de ese contraste y sus
correspondientes parámetros de significancia.

```{r EP_BAM_RES_TABLE, echo=FALSE}
tableContrast1 <- read.csv("./EventPointerStatsResult/bootstrapResult/ResBootstrapContrast1.csv",row.names = 1)
kable(tableContrast1[1:5,],digits=5,row.names=TRUE,align="c",caption = "Table 1: EventPointer_BAM results")
```
...
siguiente: 
![Figure 2. Output EventPointerBAM functions](ImagenFicherosResultados.drawio.png)
...
Al igual que EventPointerBAM the aim of EventPointerST is to perform
detection, classification, quantification and differential analysis
between conditions using as input data sequence alignments of RNA-seq
data. Pero el método que se lleva a cabo para este cometido se basa en
aquellos eventos previamente conocidos y anotados que son aportados en
un formato de anotación GTF. Por este motivo el paso previo de
alineamiento de secuencias es llevado a cabo usando pseudoalineadores
que se basan en la anotación de los transcritos para la asignación de
abundancias relativas. La limitacion a la identificación de eventos
anotados incrementa en gran medida la velocidad computacional del
algoritmo y a su vez lo hace más trasladable entre diferentes
secuenciaciones de RNA-seq ya que los eventos detectados no serán
dependientes de la secuenciación en si sino de las anotaciones
proporcionadas previamente sobre la que se hará la identificación
inicial de los eventos. La identificación previa de los eventos en base
a la anotación evita que el ruido de la secuenciación afecte a la
identificación provocando falsos positivos.
...
EventPointer has two pipelines for RNA-Seq analysis: Analysis from BAM
files and analysis from a transcriptome reference. These two pipelines
